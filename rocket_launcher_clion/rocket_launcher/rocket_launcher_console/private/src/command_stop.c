...
celix_status_t commandStop_create(bundle_context_pt bundle_context_ptr, command_pt *command_ptr) {
    ...
    *command_ptr = calloc(1, sizeof(**command_ptr));
    ...
    if (status == CELIX_SUCCESS) {
        ...
        (*command_ptr)->name = "rocketLauncher_stop";
        (*command_ptr)->shortDescription = "Stop moving the rocketlauncher";
        (*command_ptr)->usage = "rocketLauncher_stop";
        (*command_ptr)->executeCommand = commandStop_stop;
...
void commandStop_stop(command_pt command, char *line, void (*out)(char *), void (*err)(char *)) {
    ...
    printf("SRV %p\n", handle->rocket_launcher_service_ptr->stop);
    celixThreadMutex_lock(&handle->lock);
    handle->rocket_launcher_service_ptr->stop(handle->rocket_launcher_service_ptr->rocket_launcher_component_ptr);
